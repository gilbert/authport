...
OAuth2.prototype.request = function(original, cb) {
  ...
  if (request.method == "GET") {
    ...
    request.headers["User-Agent"] = "authport/1.0";
  }
...
OAuth2.prototype.onCode = function(req, res) {
  ...
  this.request(this.token, function(err, data) {
    ...
    if (typeof this.user === 'function') {
      this.getUserRequestOptions = this.user
    }
    if (this.getUserRequestOptions) {
      // Function is expected to return an object
      this.user = this.getUserRequestOptions(data.access_token, data)
    }

    if ( ! this.user.query ) this.user.query = {}
